  ...
  return (
    <>
      ...
      <div style={{
        ...
        background: "#0f0d09", borderRight: "1px solid #1e1a12", zIndex: 30,
              ...
              {loading && (
                <div style={{ padding: "2rem 1.25rem", textAlign: "center" }}>
                  <div style={{ display: "flex", gap: "4px", justifyContent: "center" }}>
                    {[0, 1, 2].map(n => (
                      <div key={n} style={{ width: "4px", height: "4px", borderRadius: "50%", background: "#c8a96e", animation: `pulse 1.2s ease-in-out ${n * 0.2}s infinite` }} />
                    ))}
                  </div>
                </div>
              )}
                ...
                <div key={date}>
                  ...
                  {dateSessions.map(session => (
                    <button key={session.session_id} onClick={() => handleLoadSession(session.session_id)}
                      ...
                      style={{ width: "100%", textAlign: "left", padding: "0.65rem 1.25rem", background: session.session_id === currentSessionId ? "rgba(200,169,110,0.08)" : "transparent", border: "none", borderLeft: `2px solid ${session.session_id === currentSessionId ? "#c8a96e" : "transparent"}`, cursor: "pointer", transition: "all 0.15s", display: "block" }}
                      onMouseEnter={(e) => { if (session.session_id !== currentSessionId) e.currentTarget.style.background = "rgba(200,169,110,0.04)"; }}
